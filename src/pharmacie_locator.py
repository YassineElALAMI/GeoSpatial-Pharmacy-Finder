...
class PharmacyFinder:
    ...
    def get_detailed_location(self) -> UserLocation:
        """Get detailed user location using a fixed position"""
        fixed_latitude =  34.03490160825201
        fixed_longitude = -4.9764940779744435
        address = "Fès, Morocco (fixed position)"
        method = "Fixed position"

        logger.info(f"Using fixed position: Latitude {fixed_latitude}, Longitude {fixed_longitude}")
        return UserLocation(
    ...
    def estimate_walking_time(self, distance_km: float) -> int:
        """Estimate walking time in minutes (average speed 5 km/h)"""
        walking_speed_kmh = 5.0
        walking_time_hours = distance_km / walking_speed_kmh
        return int(walking_time_hours * 60)
    ...
    def create_interactive_map(self, pharmacies: List[PharmacyResult]) -> str:
        """Create an interactive map with the pharmacies"""
